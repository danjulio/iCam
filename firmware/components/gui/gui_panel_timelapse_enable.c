/*
 * GUI timelapse settings enable control panel
 *
...
// API
//
void gui_panel_timelapse_enable_init(lv_obj_t* parent_cont)
{
	...
	lv_label_set_static_text(lbl_name, "Timelapse Enable");
		
	// Enable switch
	sw_enable = lv_switch_create(my_panel, NULL);
	lv_obj_add_protect(sw_enable, LV_PROTECT_CLICK_FOCUS);
	lv_obj_set_size(sw_enable, GUIPN_TIMELAPSE_ENABLE_SW_W, GUIPN_TIMELAPSE_ENABLE_SW_H);
...
void gui_panel_timelapse_enable_set_active(bool is_active)
{
	if (is_active) {
		// Get the current value
		if (gui_state.timelapse_enable) {
...
static void _cb_sw_enable(lv_obj_t* obj, lv_event_t event)
{
	if (event == LV_EVENT_VALUE_CHANGED) {
		gui_state.timelapse_enable = lv_switch_get_state(obj);
