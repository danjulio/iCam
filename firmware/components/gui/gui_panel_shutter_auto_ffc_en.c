/*
 * GUI shutter Auto FFC enable control panel
 *
...
// API
//
void gui_panel_shutter_auto_ffc_en_init(lv_obj_t* parent_cont)
{
	...
	lv_label_set_static_text(lbl_name, "Auto Shutter Enable");
		
	// Enable switch
	sw_enable = lv_switch_create(my_panel, NULL);
	lv_obj_add_protect(sw_enable, LV_PROTECT_CLICK_FOCUS);
	lv_obj_set_size(sw_enable, GUIPN_SHUTTER_AUTO_FFC_EN_SW_W, GUIPN_SHUTTER_AUTO_FFC_EN_SW_H);
...
void gui_panel_shutter_auto_ffc_en_set_active(bool is_active)
{
	if (is_active) {
		// Get the current switch value
		if (gui_state.auto_ffc_en) {
...
static void _cb_sw_enable(lv_obj_t* obj, lv_event_t event)
{
	if (event == LV_EVENT_VALUE_CHANGED) {
		gui_state.auto_ffc_en = lv_switch_get_state(obj);
