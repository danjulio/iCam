/*
 * GUI ambient settings default ambient temp control panel
 *
...
// Value string for "0" to "XXX.X" (theoretically)
#define MAX_DIST_LEN 5
...
// State
static char val_buf[MAX_DIST_LEN+1];
static char working_val_buf[MAX_DIST_LEN+1];
...
// API
//
void gui_panel_ambient_distance_init(lv_obj_t* parent_cont)
{
	...
	lv_label_set_static_text(lbl_name, "Distance");
	...
	lv_obj_set_height(ctrl_assy, GUIPN_AMBIENT_DISTANCE_BTN_H + 10);
	lv_obj_set_width(ctrl_assy, GUIPN_AMBIENT_DISTANCE_BTN_W + GUIPN_AMBIENT_DISTANCE_UNIT_W);
	...
	lv_obj_set_size(btn_val, GUIPN_AMBIENT_DISTANCE_BTN_W, GUIPN_AMBIENT_DISTANCE_BTN_H);
	...
	lv_obj_set_width(lbl_val, GUIPN_AMBIENT_DISTANCE_BTN_W);
	...
	lv_obj_set_width(lbl_val_units, GUIPN_AMBIENT_DISTANCE_UNIT_W);
	lv_obj_align(lbl_val_units, btn_val, LV_ALIGN_OUT_RIGHT_MID, 5, 0);
	if (gui_state.temp_unit_C) {
		lv_label_set_static_text(lbl_val_units, "m");
	} else {
		lv_label_set_static_text(lbl_val_units, "ft");
	}
...
void gui_panel_ambient_distance_set_active(bool is_active)
{
	if (is_active) {
		// Get the current value
		sprintf(val_buf, "%1.1f", gui_dist_to_disp_dist((int16_t) gui_state.distance, &gui_state));
...
static void _cb_btn_val(lv_obj_t* obj, lv_event_t event)
{
	if (event == LV_EVENT_CLICKED) {
		if (!gui_keypad_displayed()) {
			...
			gui_display_keypad(my_parent_page, GUI_KEYPAD_TYPE_NUMERIC, "Enter Distance", working_val_buf, MAX_DIST_LEN, _cb_keypad_handler);
		}
	}
}
...
{	
	...
	if (kp_event == GUI_KEYPAD_EVENT_CLOSE_ACCEPT) {
		if (strlen(working_val_buf) == 0) {
			...
			// Convert the string back to a numeric distance in m
			d = atof(working_val_buf);
			...
			if ((d <= 0) || (d > 20000)) {
				gui_display_message_box(my_parent_page, "Distance out of range 0 to 200m", GUI_MSG_BOX_1_BTN, NULL);
			} else {
				gui_state.distance = (uint32_t) round(d);
				sprintf(val_buf, "%1.1f", gui_dist_to_disp_dist((int16_t) gui_state.distance, &gui_state));
