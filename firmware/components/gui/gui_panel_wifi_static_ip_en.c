...
void gui_panel_wifi_static_ip_en_init(lv_obj_t* parent_cont)
{
	...
	lv_label_set_static_text(lbl_name, "STA Static IP Enable");
		
	// Enable switch
	sw_enable = lv_switch_create(my_panel, NULL);
	lv_obj_add_protect(sw_enable, LV_PROTECT_CLICK_FOCUS);
	lv_obj_set_size(sw_enable, GUIPN_WIFI_STATIC_IP_EN_SW_W, GUIPN_WIFI_STATIC_IP_EN_SW_H);
...
void gui_panel_wifi_static_ip_en_set_active(bool is_active)
{
	if (is_active) {
		...
		lv_obj_set_state(sw_enable, gui_state.sta_mode ? LV_STATE_DEFAULT : LV_STATE_DISABLED);
	}
}
...
static void _cb_sw_enable(lv_obj_t* obj, lv_event_t event)
{
	if (event == LV_EVENT_VALUE_CHANGED) {
		gui_state.sta_static_ip = lv_switch_get_state(obj);
