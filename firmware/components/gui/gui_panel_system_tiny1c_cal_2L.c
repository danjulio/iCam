...
static float blackbody_temp = GUIPN_SYSTEM_TINY1C_CAL_2L_TEMP;
...
void gui_panel_system_tiny1c_cal_2L_init(lv_obj_t* parent_cont)
{
	...
	lv_label_set_static_text(lbl_name, "L 2 point calibration");
	
	// Restore Factory Defaults Button
	btn_cal = lv_btn_create(row1, NULL);
	lv_obj_add_protect(btn_cal, LV_PROTECT_CLICK_FOCUS);
	lv_obj_set_size(btn_cal, GUIPN_SYSTEM_TINY1C_CAL_2L_BTN_W, GUIPN_SYSTEM_TINY1C_CAL_2L_BTN_H);
	...
	lv_obj_set_height(ctrl_assy, GUIPN_SYSTEM_TINY1C_CAL_2L_BTN_H + 10);
	lv_obj_set_width(ctrl_assy, GUIPN_SYSTEM_TINY1C_CAL_2L_BTN_W + GUIPN_SYSTEM_TINY1C_CAL_2L_UNIT_W);
	...
	lv_obj_set_size(btn_val, GUIPN_SYSTEM_TINY1C_CAL_2L_BTN_W, GUIPN_SYSTEM_TINY1C_CAL_2L_BTN_H);
	...
	lv_obj_set_width(lbl_val, GUIPN_SYSTEM_TINY1C_CAL_2L_BTN_W);
	...
	lv_obj_set_width(lbl_val_units, GUIPN_SYSTEM_TINY1C_CAL_2L_UNIT_W);
...
static void _cb_messagebox(int btn_id)
{
	if (btn_id == GUI_MSG_BOX_BTN_AFFIRM) {
		...
		gui_send_activity_command(CMD_CTRL_ACT_TINY1C_CAL_2L, (int32_t) round(blackbody_temp + 273.15), my_parent_page, "Calibration in progress");
	}
}
