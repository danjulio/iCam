/*
 * GUI shutter minimum interval control panel
 *
...
// API
//
void gui_panel_shutter_min_interval_init(lv_obj_t* parent_cont)
{
	...
	lv_label_set_static_text(lbl_name, "Minimum Shutter Interval");
	...
	lv_obj_set_height(ctrl_assy, GUIPN_SHUTTER_MIN_INTERVAL_BTN_H + 10);
	lv_obj_set_width(ctrl_assy, GUIPN_SHUTTER_MIN_INTERVAL_BTN_W + GUIPN_SHUTTER_MIN_INTERVAL_UNIT_W);
	...
	lv_obj_set_size(btn_val, GUIPN_SHUTTER_MIN_INTERVAL_BTN_W, GUIPN_SHUTTER_MIN_INTERVAL_BTN_H);
	...
	lv_obj_set_width(lbl_val, GUIPN_SHUTTER_MIN_INTERVAL_BTN_W);
	...
	lv_obj_set_width(lbl_val_units, GUIPN_SHUTTER_MIN_INTERVAL_UNIT_W);
...
void gui_panel_shutter_min_interval_set_active(bool is_active)
{
	if (is_active) {
		// Get the current value
#ifdef ESP_PLATFORM
		sprintf(val_buf, "%lu", gui_state.min_ffc_interval);
#else
		sprintf(val_buf, "%u", gui_state.min_ffc_interval);
#endif
...
static void _cb_btn_val(lv_obj_t* obj, lv_event_t event)
{
	if (event == LV_EVENT_CLICKED) {
		if (!gui_keypad_displayed()) {
			...
			gui_display_keypad(my_parent_page, GUI_KEYPAD_TYPE_NUMERIC, "Enter Minimum FFC Interval (secs)", working_val_buf, MAX_VAL_LEN, _cb_keypad_handler);
		}
	}
}
...
{	
	...
	if (kp_event == GUI_KEYPAD_EVENT_CLOSE_ACCEPT) {
		if (strlen(working_val_buf) == 0) {
			...
			if ((t < 5) || (t > 655)) {
				gui_display_message_box(my_parent_page, "Interval out of range 5 to 655 seconds", GUI_MSG_BOX_1_BTN, NULL);
			} else {
#ifdef ESP_PLATFORM
