/*
 * GUI settings thermal image brightness control panel
 *
...
// API
//
void gui_panel_settings_brightness_init(lv_obj_t* parent_cont)
{
	...
	lv_obj_set_width(sld_brightness, GUIPN_SETTINGS_BRIGHTNESS_SLD_W);
...
void gui_panel_settings_brightness_set_active(bool is_active)
{
	if (is_active) {
		// Get the current brightness
		cur_percent = (int) gui_state.brightness;
		...
		if (prev_active) {
			// Update brightness if there was a change
			if (cur_percent != (int) gui_state.brightness) {
				gui_state.brightness = (uint32_t) cur_percent;
				(void) cmd_send_int32(CMD_SET, CMD_BRIGHTNESS, (int32_t) cur_percent);
			}
		}
	}
