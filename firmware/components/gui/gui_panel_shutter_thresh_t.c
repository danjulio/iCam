/*
 * GUI shutter temperature change threshold control panel
 *
...
// API
//
void gui_panel_shutter_thresh_t_init(lv_obj_t* parent_cont)
{
	...
	lv_label_set_static_text(lbl_name, "Shutter Delta Temp");
	...
	lv_obj_set_height(ctrl_assy, GUIPN_SHUTTER_THRESH_T_BTN_H + 10);
	lv_obj_set_width(ctrl_assy, GUIPN_SHUTTER_THRESH_T_BTN_W + GUIPN_SHUTTER_THRESH_T_UNIT_W);
	...
	lv_obj_set_size(btn_val, GUIPN_SHUTTER_THRESH_T_BTN_W, GUIPN_SHUTTER_THRESH_T_BTN_H);
	...
	lv_obj_set_width(lbl_val, GUIPN_SHUTTER_THRESH_T_BTN_W);
	...
	lv_obj_set_width(lbl_val_units, GUIPN_SHUTTER_THRESH_T_UNIT_W);
...
void gui_panel_shutter_thresh_t_set_active(bool is_active)
{
	float t;
	
	if (is_active) {
		// Get the current value
		t = (float) gui_state.ffc_temp_threshold_x10 / 10.0;
		if (!gui_state.temp_unit_C) {
			t = t * 9.0 / 5.0;
		}
		sprintf(val_buf, "%1.1f", t);
...
static void _cb_btn_val(lv_obj_t* obj, lv_event_t event)
{
	if (event == LV_EVENT_CLICKED) {
		if (!gui_keypad_displayed()) {
			...
			gui_display_keypad(my_parent_page, GUI_KEYPAD_TYPE_NUMERIC, "Enter Delta Temp", working_val_buf, MAX_VAL_LEN, _cb_keypad_handler);
		}
	}
}
...
{	
	...
	if (kp_event == GUI_KEYPAD_EVENT_CLOSE_ACCEPT) {
		if (strlen(working_val_buf) == 0) {
			...
			if ((t < 0) || (t > 10)) {
				gui_display_message_box(my_parent_page, "Temperature out of range 0 to 10 °C", GUI_MSG_BOX_1_BTN, NULL);
			} else {
				gui_state.ffc_temp_threshold_x10 = (uint32_t) (t * 10.0);
				if (!gui_state.temp_unit_C) {
					t = t * 9.0 / 5.0;
				}
				sprintf(val_buf, "%1.1f", t);
