/*
 * GUI settings timelapse control panel
 *
...
// API
//
void gui_panel_settings_timelapse_init(lv_obj_t* screen, lv_obj_t* parent_cont)
{
	...
	lv_label_set_static_text(lbl_name, "Timelapse");
	
	// Sub-page open button
	btn_open_sub_page = lv_btn_create(my_panel, NULL);
	lv_obj_set_size(btn_open_sub_page, GUIPN_SETTINGS_TIMELAPSE_BTN_W, GUIPN_SETTINGS_TIMELAPSE_BTN_H);
	...
	// Create our sub-page
	my_sub_page = gui_sub_page_timelapse_init(screen);

	// Register with our parent page
	gui_page_settings_register_panel(my_panel, my_sub_page, gui_sub_page_timelapse_set_active, gui_sub_page_timelapse_reset_screen_size);
}


void gui_panel_settings_timelapse_set_active(bool is_active)
{
