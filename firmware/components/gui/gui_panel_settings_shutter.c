/*
 * GUI settings shutter control panel
 *
...
static lv_obj_t* btn_assy;
static lv_obj_t* btn_ffc;
static lv_obj_t* lbl_btn_ffc;
...
static void _cb_btn_ffc(lv_obj_t* obj, lv_event_t event);
...
// API
//
void gui_panel_settings_shutter_init(lv_obj_t* screen, lv_obj_t* parent_cont)
{
	...
	lv_label_set_static_text(lbl_name, "Shutter Settings");
	
	// Button assembly (sub_page open and FFC buttons so my_panel container spaces it correctly)
	btn_assy = lv_obj_create(my_panel, NULL);
	lv_obj_set_click(btn_assy, false);
	lv_obj_set_height(btn_assy, GUIPN_SETTINGS_SHUTTER_BTN_H + 10);
	lv_obj_set_width(btn_assy, GUIPN_SETTINGS_SHUTTER_BTN_W + GUIPN_SETTINGS_SHUTTER_FFC_W + GUIPN_SETTINGS_SHUTTER_FFC_S);
	lv_obj_set_style_local_border_width(btn_assy, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, 0);
	
	// FFC button (to the left of the sub-page open button)
	btn_ffc = lv_btn_create(btn_assy, NULL);
	lv_obj_set_size(btn_ffc, GUIPN_SETTINGS_SHUTTER_FFC_W, GUIPN_SETTINGS_SHUTTER_FFC_H);
	lv_obj_align(btn_ffc, btn_assy, LV_ALIGN_IN_LEFT_MID, GUIPN_SETTINGS_SHUTTER_FFC_S, 0);
	lv_obj_add_protect(btn_ffc, LV_PROTECT_CLICK_FOCUS);
	lv_obj_set_event_cb(btn_ffc, _cb_btn_ffc);
	
	// FFC Button Label
	lbl_btn_ffc = lv_label_create(btn_ffc, NULL);
	lv_label_set_align(lbl_btn_ffc, LV_LABEL_ALIGN_CENTER);
	lv_label_set_static_text(lbl_btn_ffc, "FFC");
	
	// Sub-page open button
	btn_open_sub_page = lv_btn_create(btn_assy, NULL);
	lv_obj_set_size(btn_open_sub_page, GUIPN_SETTINGS_SHUTTER_BTN_W, GUIPN_SETTINGS_SHUTTER_BTN_H);
	lv_obj_align(btn_open_sub_page, btn_assy, LV_ALIGN_IN_RIGHT_MID, 0, 0);
	...
	// Create our sub-page
	my_sub_page = gui_sub_page_shutter_init(screen);

	// Register with our parent page
	gui_page_settings_register_panel(my_panel, my_sub_page, gui_sub_page_shutter_set_active, gui_sub_page_shutter_reset_screen_size);
}


void gui_panel_settings_shutter_set_active(bool is_active)
{
