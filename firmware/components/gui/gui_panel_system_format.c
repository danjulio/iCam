...
//
static void _cb_btn_format(lv_obj_t* obj, lv_event_t event);
...
{
	// Enable updating card present status
	gui_enable_card_present_updating(is_active);
}
...
static void _cb_btn_format(lv_obj_t* obj, lv_event_t event)
{
	if (event == LV_EVENT_CLICKED) {
		// We check for a popup displayed so that this won't trigger if pressed while some
		// other controls keypad or messagebox is displayed
		if (!gui_popup_displayed()) {
			if (gui_state.card_present) {
				// Display the "are you really sure?" messagebox
				gui_display_message_box(my_parent_page, "Format SD Card? It will be erased.", GUI_MSG_BOX_2_BTN, _cb_messagebox);
			} else {
				gui_display_message_box(my_parent_page, "No SD Card present", GUI_MSG_BOX_1_BTN, NULL);
			}
		}
	}
}
...
static void _cb_messagebox(int btn_id)
{
	if (btn_id == GUI_MSG_BOX_BTN_AFFIRM) {
		...
		gui_send_activity_command(CMD_CTRL_ACT_SD_FORMAT, 0, my_parent_page, "Format in progress");
	}
}
