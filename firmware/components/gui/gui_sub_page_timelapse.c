...
#define CMD_TIMELAPSE_CFG_LEN 10


//
// Local variables
//
static const char* TAG = "gui_sub_page_timelapse";
...
static bool _send_timelapse_info();
...
lv_obj_t* gui_sub_page_timelapse_init(lv_obj_t* screen)
{
	...
	lv_obj_set_pos(btn_back, GUISP_TIMELAPSE_BACK_X, GUISP_TIMELAPSE_BACK_Y);
	lv_obj_set_size(btn_back, GUISP_TIMELAPSE_BACK_W, GUISP_TIMELAPSE_BACK_H);
	...
	lv_obj_set_pos(lbl_title, GUISP_TIMELAPSE_TITLE_X, GUISP_TIMELAPSE_TITLE_Y);
	lv_label_set_static_text(lbl_title, "Timelapse");
	...
	gui_panel_timelapse_enable_init(page_controls);
	gui_panel_timelapse_interval_init(page_controls);
	gui_panel_timelapse_num_img_init(page_controls);
	gui_panel_timelapse_notify_init(page_controls);
...
void gui_sub_page_timelapse_set_active(bool is_active)
{
	...
	gui_panel_timelapse_enable_set_active(is_active);
	gui_panel_timelapse_interval_set_active(is_active);
	gui_panel_timelapse_num_img_set_active(is_active);
	gui_panel_timelapse_notify_set_active(is_active);
...
void gui_sub_page_timelapse_reset_screen_size(uint16_t page_w, uint16_t page_h)
{
	...
	// Set title max width
	lv_obj_set_width(lbl_title, page_w - (2 * GUISP_TIMELAPSE_TITLE_X));
	
	// Set control page dimensions
	lv_obj_set_size(page_controls, page_w, page_h - GUISP_TIMELAPSE_CONTROL_Y);
...
static bool _send_timelapse_info()
{
	uint8_t buf[CMD_TIMELAPSE_CFG_LEN];
	
	// Note: this routine must match order and size of cmd_handler_set_timelapse
	// (cmd_handlers.c)
	buf[0] = (uint8_t) gui_state.timelapse_enable;
	buf[1] = (uint8_t) gui_state.timelapse_notify;
	*(uint32_t*)&buf[2] = htonl((uint32_t) gui_state.timelapse_interval_sec);
	*(uint32_t*)&buf[6] = htonl((uint32_t) gui_state.timelapse_num_img);
	
	return cmd_send_binary(CMD_SET, CMD_TIMELAPSE_CFG, CMD_TIMELAPSE_CFG_LEN, buf);
}
