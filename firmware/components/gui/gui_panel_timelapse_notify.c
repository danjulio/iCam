...
void gui_panel_timelapse_notify_init(lv_obj_t* parent_cont)
{
	...
	lv_label_set_static_text(lbl_name, "Save Notification Enable");
		
	// Enable switch
	sw_enable = lv_switch_create(my_panel, NULL);
	lv_obj_add_protect(sw_enable, LV_PROTECT_CLICK_FOCUS);
	lv_obj_set_size(sw_enable, GUIPN_TIMELAPSE_NOTIFY_SW_W, GUIPN_TIMELAPSE_NOTIFY_SW_H);
...
static void _cb_sw_enable(lv_obj_t* obj, lv_event_t event)
{
	if (event == LV_EVENT_VALUE_CHANGED) {
		gui_state.timelapse_notify = lv_switch_get_state(obj);
