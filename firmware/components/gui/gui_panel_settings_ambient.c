...
void gui_panel_settings_ambient_init(lv_obj_t* screen, lv_obj_t* parent_cont)
{
	...
	lv_label_set_static_text(lbl_name, "Ambient Correction");
	
	// Sub-page open button
	btn_open_sub_page = lv_btn_create(my_panel, NULL);
	lv_obj_set_size(btn_open_sub_page, GUIPN_SETTINGS_AMBIENT_BTN_W, GUIPN_SETTINGS_AMBIENT_BTN_H);
	...
	// Create our sub-page
	my_sub_page = gui_sub_page_ambient_init(screen);

	// Register with our parent page
	gui_page_settings_register_panel(my_panel, my_sub_page, gui_sub_page_ambient_set_active, gui_sub_page_ambient_reset_screen_size);
}
