...
void gui_panel_wifi_sta_netmask_init(lv_obj_t* parent_cont)
{
	...
	lv_label_set_static_text(lbl_name, "STA Static Netmask");
	
	// IP Address Button
	btn_ip_addr = lv_btn_create(my_panel, NULL);
	lv_obj_set_y(btn_ip_addr, 5);
	lv_obj_add_protect(btn_ip_addr, LV_PROTECT_CLICK_FOCUS);
	lv_obj_set_size(btn_ip_addr, GUIPN_WIFI_STA_NETMASK_BTN_W, GUIPN_WIFI_STA_NETMASK_BTN_H);
	...
	lv_obj_set_width(lbl_btn_ip_addr, GUIPN_WIFI_STA_NETMASK_BTN_W);
...
{
	if (is_active) {
		gui_print_ipv4_addr(val_buf, gui_state.sta_netmask);
...
static void _cb_btn_ip_addr(lv_obj_t* obj, lv_event_t event)
{
	if (event == LV_EVENT_CLICKED) {
		...
		if (!gui_keypad_displayed()) {
			...
			gui_display_keypad(my_parent_page, GUI_KEYPAD_TYPE_NUMERIC, "Enter IPV4 netmask", working_val_buf, MAX_IP_STRING_LEN, _cb_keypad_handler);
		}
	}
}
	...
	if (kp_event == GUI_KEYPAD_EVENT_CLOSE_ACCEPT) {
		if (!gui_parse_ipv4_addr_string(working_val_buf, temp_ip)) {
			...
			if (strcmp(val_buf, working_val_buf) != 0) {
				...
				for (int i=0; i<4; i++) {
					gui_state.sta_netmask[i] = temp_ip[i];
				}
