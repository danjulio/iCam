...
{
    const lv_coord_t hres = disp_drv->hor_res;
    const lv_coord_t vres = disp_drv->ver_res;
...
static void window_create(monitor_t * m)
{
    ...
    m->window = SDL_CreateWindow("iCamMini",
    ...
    memset(m->tft_fb, 0x00, SDL_HOR_RES * SDL_VER_RES * sizeof(uint32_t));
#endif
